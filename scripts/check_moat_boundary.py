...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awask
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awgym-*.whl dist/*.tar.gz ...]
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awask-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awask/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awask/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awask/x.py": "# MOAT001 says so\n"}))), True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awask-0.0.0.tar.gz"
